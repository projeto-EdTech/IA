createQuestion({
    id: 1,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "INSTRUÇÃO: As questões 1 e 2 referem-se ao",
      subItens: [{"titulo":"Texto 1","conteudo":"a seguir. Leia-o com atenção para respondê-las."},{"titulo":"TEXTO 1","conteudo":"Como evitar ou tratar a depressão? Com exercício físico, oras\nA ciência confirma o papel da atividade física na prevenção e no controle da depressão, um mal que se alastra em proporções epidêmicas\nÉ triste dizer, mas a depressão está no ar. Segundo a Organização Mundial da Saúde (OMS), mais de 300 milhões de pessoas sofrem com o problema atualmente – houve um aumento de 18% entre 2005 e 2015. E a tendência é que esse número não pare de crescer. Alarmada, a própria OMS lançou um apelo aos países: é hora de todos incluírem o tema em suas políticas públicas de saúde. Acontece que não basta dar remédio a esse montão de gente que está com a mente em apuros. A solução, tanto em matéria de prevenção como no tratamento, engloba outros ajustes, como mudanças de hábito. Nesse sentido, pode apostar: teremos de suar a camisa para reverter a situação. Literalmente.\nNovos estudos reforçam o poder da atividade física para o bem-estar psicológico. A ponto de o exercício virar prescrição para pessoas deprimidas (ao lado da psicoterapia e dos medicamentos). “Hoje, em toda especialidade, qualquer médico vai listar uma série de benefícios das atividades esportivas. Na psiquiatria, isso se aplica à depressão”, diz o psiquiatra Marcelo Fleck, chefe do Departamento de Psiquiatria e Medicina Legal da Universidade Federal do Rio Grande do Sul.\nEmbora os impactos do esforço físico na esfera mental sejam um campo de pesquisa novo, multiplicam-se evidências de que caminhar, pedalar e malhar melhoram a qualidade de vida de quem anda pra baixo. “É provável que o efeito do exercício se aproxime muito ao dos antidepressivos”, conta Fleck.\nSabe-se que os esportes promovem a liberação de endorfina, o hormônio do prazer, e de outros neurotransmissores por trás da sensação de bem-estar. Experimentos recentes mostram que suar a camisa também estimula o crescimento de células nervosas no hipocampo, região do cérebro que rege a memória e o humor. Um alento e tanto se você pensar que essa estrutura costuma ser menor entre os sujeitos deprimidos.\nEsse estímulo aos neurônios é o que ajuda a entender os reflexos positivos de longo prazo – vai muito além, portanto, da sensação imediata de prazer e dever cumprido após a academia. “A liberação de hormônios não é o que faz a pessoa melhorar. A superação da doença tem a ver com a regeneração neuronal”, revela o educador físico e doutor em psiquiatria Felipe Schuch, do Centro Universitário La Salle, em Canoas (RS). Só que esse efeito terapêutico depende de regularidade.\nBRUM, Maurício; ORTIZ, Juan; KANITZ, Henrique. Como evitar ou tratar a depressão? Com exercício físico, oras. Disponível em: https://saude.abril.com.br/fitness/como-evitar-ou-tratar-a-depressao-com-exercicio/. Acesso: 11 dez. 2018.\n\nEm relação a esse texto, é CORRETO afirmar que ele:"}]
    },
    options: ["Tem por finalidade explicar que a endorfina, o hormônio do prazer, e outros neurotransmissores que estão por trás da sensação de bem-estar são liberados quando as pessoas saem às ruas para praticar atividades, como caminhar, pedalar e malhar.","Objetiva principalmente mostrar que a depressão atinge mais de 300 milhões de pessoas no mundo e que a Organização Mundial da Saúde (OMS) lançou um apelo aos países, convocando todos a incluírem esse tema em suas políticas públicas de saúde.","Revela que experimentos científicos recentes confirmam o papel da atividade física na prevenção e no controle da depressão ao estimular o crescimento de células nervosas no hipocampo, região do cérebro que rege a memória e o humor.","Defende a ideia de que os impactos do esforço físico na esfera mental constituem um campo de pesquisa inovador e que, por isso, ainda prevalece, na maior parte dos países, o tratamento de pacientes com a mente em apuros por meio de remédios."],
    correctAnswer: 2,
    materia: "Língua Portuguesa – Interpretação de Texto",
    conteudo: "Língua Portuguesa – Compreensão Textual",
    imageNames: []
  }),

createQuestion({
    id: 2,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "Em relação à composição estrutural e/ou aos elementos linguísticos do",
      subItens: [{"titulo":"Texto 1","conteudo":", assinale a alternativa CORRETA:"}]
    },
    options: [" a expressão destacada pertence à classe dos advérbios e serve para sinalizar a relação semântica de oposição existente entre as ideias contidas nos enunciados que une.","Em \"Um alento e tanto se você pensar que essa estrutura costuma ser menor entre os sujeitos deprimidos\", a expressão em destaque é usada como um termo substituto que retoma a referência feita pelo nome \"cérebro\" empregado no período anterior.","No texto, as falas de autoridades no assunto tratado, o papel da atividade física na prevenção e no controle da depressão, são apresentadas por meio do discurso direto, marcado pelo uso de aspas e de verbos de elocução.","Uma característica do estilo predominantemente informal no texto é o uso de frases nominais, de períodos interrompidos e de expressões coloquiais, como “montão de gente\", \"um alento e tanto\" ou \"anda pra baixo\", por exemplo."],
    correctAnswer: 2,
    materia: "Língua Portuguesa – Elementos Linguísticos",
    conteudo: "Língua Portuguesa – Estrutura Textual",
    imageNames: []
  }),

createQuestion({
    id: 3,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "Leia o cartum a seguir:\n\nVIOLÊNCIA CRESCE NO INTERIOR DO PAÍS\nVAI PASSANDO\nA CARTEIRA,\nCAIPIRA!\nNUM FALEI\nPROCÊ QUE UM\nDIA O PROGRESSO\nCHEGAVA AQUI,\nMUIE?\nSANDRO\nDisponível em: https://www.google.com.br/search?q=cartum&.\nAcesso: 11 dez. 2018.\n\nSobre o cartum, assinale a alternativa CORRETA.",
      subItens: []
    },
    options: ["A construção perifrástica “Vai passando\" é formada pelo verbo auxiliar \"ir\" seguido do verbo principal “passar” no gerúndio, e o valor veiculado por ela é de possibilidade ou probabilidade.","A fala do assaltado contribui para a sua identificação como habitante do interior do país, por apresentar formas de expressão usuais em regiões menos urbanizadas.","A fala do assaltante se distancia da fala do assaltado pelo fato de trazer formas de expressão reservadas a ocasiões determinadas, em que os falantes incluem pessoas de outras comunidades linguísticas para sua comunicação.","As diferenças linguísticas exemplificadas no cartum tanto ocorrem em função da idade dos personagens quanto acontecem devido à influência do grupo social a que pertencem."],
    correctAnswer: 1,
    materia: "Língua Portuguesa – Variação Linguística",
    conteudo: "Língua Portuguesa – Interpretação de Cartum",
    imageNames: []
  }),

createQuestion({
    id: 4,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "INSTRUÇÃO: As questões 4 e 5 referem-se ao",
      subItens: [{"titulo":"Texto 2","conteudo":"a seguir. Leia-o com atenção para respondê-las."},{"titulo":"TEXTO 2","conteudo":"Pesquisa estuda relação entre alimentos e câncer nas redes sociais\nArtigo publicado no periódico inglês Future Science AO examina se as postagens em uma mídia social são modismo ou se há evidência científica quando o assunto é o câncer associado a alimentos funcionais.\nUma das autoras do artigo, Claudia Jurberg, bolsista de Produtividade em Pesquisa do Conselho Nacional de Desenvolvimento Científico e Tecnológico (CNPq), explica que foram analisados alimentos funcionais relacionados ao câncer no Pinterest (rede social de compartilhamento de fotos) e se havia evidência científica ou não no portal de periódicos do PubMed, que é um motor de busca de livre acesso à base de dados MEDLINE de citações e resumos de artigos de investigação em biomedicina. O Pinterest foi a mídia de escolha porque está em franca ascensão no mundo e no Brasil.\n\"Foram analisados 507 Pins, sendo 75 de alimentos associados ao câncer, compartilhados mais de 27 mil vezes, e encontramos mais de 80 mil artigos científicos sobre esses alimentos e câncer no PubMed. Em 90% dos alimentos mencionados como funcionais para o câncer, encontramos literatura científica. Os Pins são ideias que as pessoas encontram e salvam de toda a Web\", esclarece a pesquisadora.\nClaudia informa que existem cerca de 50 bilhões de pins sobre comida no Pinterest, e que o objetivo principal era investigar a relação entre postagens sobre comida e câncer no Pinterest e as evidências científicas. \"Surpreendentemente, 90% dos alimentos citados nessa mídia social também aparecem na literatura científica\". No entanto, apesar desse paralelo entre conteúdo publicado em mídia social e evidência científica, a pesquisadora diz que não foi possível identificar a exata relação dos alimentos com o câncer: se previnem, curam ou tratam. [...]\nCoordenação de Comunicação Social do CNPq. Ter, 21 Ago. 2018. Disponível em: http://www.cnpq.br/web/guest/noticiasviews/-/journal_content/56_INSTANCE_a6MO/10157/6333171. Acesso: 11 dez. 2018. [Fragmento. Adaptado].\n\nEm relação ao"},{"titulo":"Texto 2","conteudo":", assinale a alternativa CORRETA:"}]
    },
    options: ["O periódico inglês Future Science AO examina postagens em uma mídia social para verificar a relação entre o câncer e alimentos funcionais.","A pesquisa realizada teve como objetivo investigar a relação entre postagens sobre comida e câncer no Pinterest e as evidências científicas.","A autora do artigo analisou alimentos funcionais relacionados ao câncer sob orientação do Conselho Nacional de Desenvolvimento Científico e Tecnológico.","A pesquisa se baseia em alimentos funcionais divulgados nas mídias sociais porque estariam em franca ascensão no mundo e no Brasil."],
    correctAnswer: 1,
    materia: "Língua Portuguesa – Interpretação de Texto",
    conteudo: "Língua Portuguesa – Texto Científico",
    imageNames: []
  }),

createQuestion({
    id: 5,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "Em relação ao emprego dos sinais de pontuação na construção do",
      subItens: [{"titulo":"Texto 2","conteudo":", assinale a alternativa CORRETA:"}]
    },
    options: ["Em [...] “no Pinterest (rede social de compartilhamento de fotos)”, o uso dos parênteses se deve à necessidade do texto de separar o trecho que indica uma interrupção do pensamento da autora da pesquisa realizada.","Em \"Artigo publicado no periódico inglês Future Science AO examina se as postagens em uma mídia social são modismo ou se há evidência científica quando o assunto é o câncer associado a alimentos funcionais\", não há vírgulas separando orações porque o período é composto por coordenação.","Em \"Claudia informa que existem cerca de 50 bilhões de pins sobre comida no Pinterest, e que o objetivo principal era investigar a relação entre postagens sobre comida e câncer no Pinterest e as evidências científicas\", o uso da vírgula é facultativo já que as orações subordinadas coordenadas entre si se ligam pela conjunção \"e\".","Em “Os Pins são ideias que as pessoas encontram e salvam de toda a Web', esclarece a pesquisadora\", o uso da vírgula é obrigatório, uma vez que ela precede uma oração apositiva."],
    correctAnswer: 2,
    materia: "Língua Portuguesa – Pontuação",
    conteudo: "Língua Portuguesa – Sintaxe",
    imageNames: []
  }),

createQuestion({
    id: 6,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "Leia estes textos.\n\nTEXTO I\nBoato virou um gênero da comunicação\nO boato insiste em ser um gênero da comunicação.\nUm rumor pode nascer da má-fé, do mal-entendido ou de uma atrapalhada qualquer. O significado do rumor é uma conclusão precipitada, é algo que nem é verdade, mas que acaba ganhando popularidade. Falar mal de alguém é falar bem de si e de nosso ouvinte.\nO primeiro impulso é acreditar, porque: 1) confiamos em quem o transmite; 2) é fisicamente impossível verificar se é verdadeiro ou falso; 3) os meios de comunicação são sistematicamente relapsos com a verificação de seus conteúdos – e se eles fazem isso, o que nos impede?\nO boato não informa, mas ensina: mostra como uma sociedade se prepara para tomar uma posição.\nA nossa tem se aplicado na tarefa de desmantelar equipes de jornalistas e dar nome de “informação” a todo tipo de “copia e cola\" difundido pela internet como se fosse um fato verificado.\nSem tantos profissionais ocupados na verificação e com o bombardeio tão grande de dados em rede, o futuro da comunicação atual, depende, cada vez mais, do modo como vamos lidar com os rumores.\nPEREIRA Jr., Luiz Costa. Revista Língua Portuguesa. Ano 8. n. 93, jul. 2013, p.19 [Fragmento. Adaptado]\n\nTEXTO II\nNOTÍCIAS FALSAS CIRCULAM 70% MAIS DO QUE AS VERDADEIRAS NA WEB...\nMESTRE,\nCOMO EU FAÇO\nPARA ENCONTRAR\nA SABEDORIA?\nPRA\nCOMEÇAR,\nPARE DE ACREDITAR\nNAS NOTÍCIAS QUE\nVOCÊ LÊ NO FACEBOOK\nE EM OUTRAS REDES,\nSOCIAIS!\nWWW.BLOGDDAFIM.COM.BR WWW.BLOGDDAFR.COM\nCAZO\nDisponível em: https://blogdoafr.com/2018/03/25/charge-fake-news/. Acesso: 11 dez.2018.\n\nConsidere as características do boato e das notícias falsas, abordadas nos textos I e II, respectivamente. No Texto II, a recomendação apresentada no diálogo, pelo mestre em resposta à pergunta a ele dirigida – está diretamente relacionada com o seguinte argumento defendido no Texto I.",
      subItens: []
    },
    options: ["A crença em boatos parte da confiança naquilo que é veiculado pelos meios de comunicação e acessado pelos usuários.","A ignição do rumor é a conclusão precipitada, o veneno sem origem, a possibilidade que ganhou popularidade.","Os boatos não informam, ao contrário, são essenciais para o aprendizado daquelas pessoas que desejam ser mais sábias.","Os rumores, que geram boatos, têm origem na má-fé, no mal-entendido ou em quaisquer trapalhadas."],
    correctAnswer: 0,
    materia: "Língua Portuguesa – Interpretação de Textos Variados",
    conteudo: "Língua Portuguesa – Gêneros Textuais",
    imageNames: []
  }),

createQuestion({
    id: 7,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "LITERATURA BRASILEIRA\nINSTRUÇÃO: As questões 7 e 8 referem-se à obra “A barca dos amantes\" de Antônio Barreto.\n\nO livro de Antônio Barreto, “A barca dos amantes\", compreende quatro partes: 1) O homem que trazia o mar, 2) Livro das figurações, 3) Os deuses disfarçados e 4) Mar de mármore, nas quais",
      subItens: []
    },
    options: ["É narrada a história de amor, que culmina em um final feliz, entre o poeta Tomás Antônio Gonzaga e, sua eterna musa, Maria Dorotéia Joaquina de Seixas.","Há citações de trechos de poesia árcade de incógnita autoria, ao longo de uma recriação ficcional que remonta o ciclo do Ouro no final do século XVIII.","É construída uma \"colcha de retalhos romanceada\", envolvendo personagens literárias e históricas que viveram o período da insurreição mineira.","Há remissão a fatos da história colonial barroca em um romance sobre a vida na capital do Império em que nobres desejavam ter experiências marítimas."],
    correctAnswer: 2,
    materia: "Literatura Brasileira – Obras Literárias",
    conteudo: "Literatura Brasileira – Contexto Histórico",
    imageNames: []
  }),

createQuestion({
    id: 8,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "NÃO há na composição da obra “A barca dos amantes\":",
      subItens: []
    },
    options: ["Referência a uma lenda nórdica, ingrediente mítico que evidencia o caráter intertextual literário do romance.","Notas e bibliografia consultadas que remetem a fontes de pesquisa e de confirmação de informações presentes na narrativa.","Bases históricas sobre a Inconfidência e sobre Tomás Antônio Gonzaga que favorecem a verossimilhança do enredo.","Registro do lamento de pessoas que perderam entes queridos em naufrágios, denominadas pelo autor de \"órfãos do mar\"."],
    correctAnswer: 3,
    materia: "Literatura Brasileira – Obras Literárias",
    conteudo: "Literatura Brasileira – Características da Obra",
    imageNames: []
  }),

createQuestion({
    id: 9,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "Leia este poema.\n\nDIALÉTICA\nÉ claro que a vida é boa\nE a alegria, a única indizível emoção\nÉ claro que te acho linda\nEm ti bendigo o amor das coisas simples\nÉ claro que te amo\nE tenho tudo para ser feliz\nMas acontece que eu sou triste...\nMORAES, Vinícius. Disponível em:\nhttp://www.viniciusdemoraes.com.br/pt-br/poesia/poesias-avulsas/dialetica. Acesso: 11 dez. 2018.\n\nEm relação a esse texto, escrito pelo poeta e compositor Vinícius de Moraes, é CORRETO afirmar que:",
      subItens: []
    },
    options: ["O eu-lírico emprega, no primeiro verso, a palavra \"claro” que, nesse contexto, expressa a ideia de incerteza.","O elemento linguístico que confirma que a vida é realmente boa está presente no terceiro verso do poema.","A repetição em três versos, na sequência da estrutura do poema, reforça a ideia apresentada no sexto verso.","As expectativas são confirmadas, no último verso, pois o eu-lírico revela uma tristeza que ele consegue superar."],
    correctAnswer: 2,
    materia: "Literatura Brasileira – Poesia Moderna",
    conteudo: "Literatura Brasileira – Análise Poética",
    imageNames: []
  }),

createQuestion({
    id: 10,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "INSTRUÇÃO: As questões 10 e 11 referem-se à obra “O Triste fim de Policarpo Quaresma” de Lima Barreto.\n\nLeia estes fragmentos da adaptação em quadrinhos do livro \"Triste fim de Policarpo Quaresma”.\n\n★A INDIGNAÇÃO, A CONVICÇÃO PATRIÓTICA\nE O AMOR AO IDIOMA NATIVO SE\nMISTURARAM EM SUA FEBRE, E ELE\nNEM PERCEBEU QUE SE PÔS A REDIGIR\nO RELATÓRIO NAQUELA LINGUA.\n\"Makaka tuyué inti fra mandéo\ny pú cuimbišca o pé\nANTES QUE PERCEBESSE O\nQUE TINHA FEITO, CHAMARAM-\n-NO PARA RESOLVER LUM\nPROBLEMA EM OUTRA SALA,\nE O OFÍCIO SE JUNTOU A OU-\nTROS, SEGUINDO O CURSO\nNORMAL DA BUROCRACIA...\nPASSOU PELO DIRETOR\nQUE, COMO SEMPRE, NÃO\nREPAROU NO QUE ASSINAVA...\n****\n...E FOI PARAR NO MINISTÉRIO,\nONDE CAUSOU PERPLEXIDADE...\ninti hu mundeo...\n\"Makaka tukuyé\n\"Proceda-se\"\nExpediente\nLiteratura Brasileira em Quadrinhos. O triste fim de Policarpo Quaresma. Lima Barreto. Roteiro e Adaptação de Ronaldo Antonelli. Ilustrações de Francisco Vilacha. São Paulo: Escala Educacional, 2008. p.7.\n\nAs * reproduzem os textos das imagens dos quadrinhos:\n*“A INDIGNAÇÃO, A CONVICÇÃO PATRIÓTICA E O AMOR AO IDIOMA NATIVO SE MISTURAM EM SUA FEBRE, E ELE NEM PERCEBEU QUE SE PÔS A REDIGIR O RELATÓRIO NAQUELA LÍNGUA.”\n**\"ANTES QUE PERCEBESSE O QUE TINHA FEITO, CHAMARAM-NO PARA RESOLVER UM PROBLEMA EM OUTRA SALA, E O OFÍCIO SE JUNTOU A OUTROS, SEGUINDO O CURSO NORMAL DA BUROCRACIA...\"\n***\"...PASSOU PELO DIRETOR QUE, COMO SEMPRE, NÃO REPAROU NO QUE ASSINAVA...”\n****\"...E FOI PARAR NO MINISTÉRIO, ONDE CAUSOU PERPLEXIDADE...\"\n\nNesses trechos, há referência a missões que Policarpo Quaresma desejava realizar. Considerando o enredo da obra \"Triste fim de Policarpo Quaresma”, são projetos malogrados do protagonista:\nI- Defender o tupi-guarani como língua oficial do país, por ser esse o idioma que expressa a brasilidade em contraposição ao europeísmo representado pela língua portuguesa.\nII- Propor uma reforma da agricultura, pedindo suporte governamental aos pequenos produtores agrícolas, por reconhecer que lhes faltava mais de dedicação e organização.\nIII- Apoiar o governo na defesa da ordem republicana ameaçada e entregar a Floriano Peixoto um documento com as medidas a serem tomadas para resolver os problemas do país.\nEstão CORRETAS as afirmativas:",
      subItens: []
    },
    options: ["I e II apenas.","II e III apenas.","I e III apenas.","I, II e III."],
    correctAnswer: 3,
    materia: "Literatura Brasileira – Pré-Modernismo",
    conteudo: "Literatura Brasileira – Obras Literárias",
    imageNames: []
  }),

createQuestion({
    id: 11,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "O enredo do romance “Triste fim de Policarpo Quaresma\" gira em torno do personagem-título, funcionário público que:",
      subItens: []
    },
    options: ["É avesso à xenofobia e vive em um mundo bem ajustado com a realidade que o circunda.","Dedica amor intenso ao Brasil, sendo ridicularizado por seu idealismo e ufanismo exacerbado.","Não se identifica com as causas indígenas, desprezando as raízes históricas nacionais.","Visita a ilha das cobras para redigir um protesto em defesa de soldados presos injustamente."],
    correctAnswer: 1,
    materia: "Literatura Brasileira – Pré-Modernismo",
    conteudo: "Literatura Brasileira – Personagens Literários",
    imageNames: []
  }),

createQuestion({
    id: 12,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "Gênero literário é a categoria à qual pertence uma obra com base em suas características predominantes que possibilitam o agrupamento por semelhanças.\n\nConsiderando a classificação dos gêneros literários, assinale V para as afirmativas verdadeiras e F para as falsas.\n\n() Gênero lírico caracteriza-se pela manifestação do eu-interior do poeta e pela exposição de sua subjetividade de sentimentos e emoções vivenciados em seu mundo íntimo.\n\n() Gênero dramático caracteriza-se pela narração (em versos) de grandes feitos históricos em textos em que a presença de um herói representa a força do coletivo.\n\n() Gênero épico caracteriza-se por textos escritos para serem encenados em que o autor utiliza o discurso direto e dá voz a seus personagens sem a intermediação de um narrador.\n\nAssinale a alternativa que apresenta a sequência CORRETA:",
      subItens: []
    },
    options: ["VVV.","VFF.","FVV.","FFF."],
    correctAnswer: 1,
    materia: "Literatura Brasileira – Gêneros Literários",
    conteudo: "Língua Portuguesa – Tipologia Textual",
    imageNames: []
  }),

createQuestion({
    id: 13,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "FÍSICA\nQUESTÃO 13\nApós estudar física exaustivamente para as provas de vestibular, Lívia sentiu-se mal e precisou receber a visita de um médico.\n\nEU VEJO\nMOVIMENTOS\nCIRCULARES.\nCOM QUE\nFREQUÊNCIA?\nω/2π\nHTTP://WWW.EFEITOJOULE.COM/\nDisponível: https://www.efeitojoule.com/2011/04/vestibulario-tirinhas-do-vestibular-de.html\nAcesso: 11 dez. 2018.\n\nCom base nas informações do diálogo apresentado e considerando uma roda que gire em torno do seu próprio eixo com velocidade angular (w) constante, o período de rotação dessa roda é dado por:",
      subItens: []
    },
    options: ["2. (ω.π)-1","2.π.ω-1.","ω.2.π","ω.(2.π)-1"],
    correctAnswer: 1,
    materia: "Física – Movimento Circular Uniforme",
    conteudo: "Física – Período e Frequência",
    imageNames: []
  }),

createQuestion({
    id: 14,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 14\n\"Fundado em 2002 pelo Prêmio Nobel Carl Wieman, o projeto PhET Simulações Interativas da Universidade de Colorado Boulder (EUA) cria simulações interativas gratuitas de matemática e ciências. As simulações PhET baseiam-se em extensa pesquisa em educação e envolvem os alunos através de um ambiente intuitivo, estilo jogo, onde os alunos aprendem através da exploração e da descoberta\".\nDisponível em: https://phet.colorado.edu/pt_BR/. Acesso: 11 dez. 2018.\n\nA figura a seguir foi obtida pelo PhET, sendo que duas partículas A e B, eletricamente carregadas, foram colocadas em uma determinada região do espaço. As setas indicam a direção e o sentido das linhas de força do vetor campo elétrico do sistema.\n\$1_{1}$\nA\nB\n\nA respeito das cargas elétricas A e B, é CORRETO afirmar que:",
      subItens: []
    },
    options: ["Ambas são eletricamente positivas.","Ambas são eletricamente negativas.","B é eletricamente positiva e A é negativa.","A é eletricamente positiva e B é negativa."],
    correctAnswer: 3,
    materia: "Física – Eletrostática",
    conteudo: "Física – Campo Elétrico",
    imageNames: []
  }),

createQuestion({
    id: 15,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 15\nPara calcular o preço do consumo elétrico de seu banho, Roberta utiliza suas aulas de física para a realização dessa tarefa. Primeiramente, ela verifica que, no manual de instruções de seu chuveiro, a potência é de 5.000W quando ligado em 127V. Além disso, ela verifica em sua conta de energia que o preço de 1kWh é igual a R$ 0,70.\nSupondo que Roberta tome um banho de 15 minutos por dia, assinale a alternativa que apresenta o custo aproximado de cada banho.\n(Considere que o chuveiro esteja ligado corretamente em 127V.)",
      subItens: []
    },
    options: ["R$ 0,47.","R$ 0,75.","R$ 0,88.","R$ 1,25."],
    correctAnswer: 2,
    materia: "Física – Eletrodinâmica",
    conteudo: "Física – Potência Elétrica",
    imageNames: []
  }),

createQuestion({
    id: 16,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 16\nAntes de viajar, o motorista calibrou os pneus do seu carro a uma pressão de 30psi quando a temperatura dos pneus era de 27°C. Durante a viagem, após parar em um posto de gasolina, o motorista percebeu que os pneus estavam aquecidos. Ao conferir a calibragem, o motorista verificou que a pressão dos pneus era de 32psi.\nConsiderando a dilatação do pneu desprezível e o ar dentro dos pneus como um gás ideal, assinale a alternativa que MELHOR representa a temperatura mais próxima dos pneus.",
      subItens: []
    },
    options: ["29°C.","38°C.","47°C.","52°C."],
    correctAnswer: 2,
    materia: "Física – Termodinâmica",
    conteudo: "Física – Gases Ideais",
    imageNames: []
  }),

createQuestion({
    id: 17,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 17\nAo posicionar a mão à frente de um espelho esférico, Alice verificou a imagem da sua mão conforme a figura a seguir:\n\nDisponível em:\nhttps://www.pasco.com/images/products/se/se7573_enlrg_169161.jpg\nAcesso: 11 dez. 2018.\n\nO tipo de imagem formada da mão e o espelho utilizado são, respectivamente:",
      subItens: []
    },
    options: ["Virtual e côncavo.","Virtual e convexo.","Real e convexo.","Real e côncavo."],
    correctAnswer: 0,
    materia: "Física – Óptica Geométrica",
    conteudo: "Física – Espelhos Esféricos",
    imageNames: []
  }),

createQuestion({
    id: 18,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 18\nLeia o trecho a seguir:\nO efeito fotoelétrico foi descoberto em 1886 pelo físico alemão Heinrich Hertz (1857-1894). Na ocasião, Hertz percebeu que a incidência da luz ultravioleta em chapas metálicas auxiliava a produção de faíscas. A explicação teórica para o efeito fotoelétrico, entretanto, só foi apresentada pelo físico alemão Albert Einstein em 1905.\nA dúvida que existia na época estava relacionada com a energia cinética dos elétrons que eram ejetados do metal: essa grandeza não dependia do(a) __________ da luz incidente. Einstein percebeu que o agente responsável pela ejeção de cada elétron era um único fóton, uma partícula de luz que transferia aos elétrons uma parte de sua energia, ejetando-o do material, desde que seu(sua) __________ fosse grande o suficiente para tal.\nDisponível em: https://brasilescola.uol.com.br/o-que-e/fisica/o-que-e-efeito-fotoeletrico.htm\nAcesso: 11 dez. 2018. (Fragmento: Adaptado).\n\nAssinale a alternativa que preenche CORRETAMENTE as lacunas.",
      subItens: []
    },
    options: ["frequência – comprimento de onda.","comprimento de onda – intensidade.","intensidade – frequência.","comprimento de onda – frequência."],
    correctAnswer: 2,
    materia: "Física – Física Moderna",
    conteudo: "Física – Efeito Fotoelétrico",
    imageNames: []
  }),

createQuestion({
    id: 19,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "HISTÓRIA\nQUESTÃO 19\nLeia o fragmento a seguir:\n\"Desde o início de 1964, em torno das propostas de reformas de base, intensificavam-se as manifestações de apoio e de repulsa ao governo de João Goulart. Em 31 de março, um alto escalão de oficiais do Exército, com o apoio de vários governantes, como Magalhães Pinto, de Minas Gerais, Carlos Lacerda, da Guanabara, e Adhemar de Barros, de São Paulo, rebelou-se contra o governo de Jango. O golpe teve um desfecho rápido e bem-sucedido. Culminou com a deposição do presidente João Goulart, que deixou Brasília e se dirigiu para o Rio Grande do Sul. Era o início da ditadura comandada pelos militares também chamada de 'Anos de Chumbo\".\nFONTE: Vicentino, Cláudio. Projeto Radix: História. 2ª ed. – São Paulo: Scipione, 2012. [Fragmento: Adaptado]\n\nSobre o período a que se refere o fragmento apresentado, assinale V para as afirmativas verdadeiras e F para as falsas.\n\n() Na área econômica, os governos militares promoveram a abertura do mercado ao capital e às empresas estrangeiras, ampliando a internacionalização da economia.\n\n() A ditadura militar formou um eficiente serviço de propaganda, que buscava despertar o sentido de patriotismo na população com slogans como \"Este é um país que vai pra frente”; “Ninguém segura esse país”; “Brasil, ame-o ou deixe-o\".\n\n() A junta formada após a deposição de Jango teve como primeira medida a decretação do Ato Institucional nº1 (AI-1) que estabeleceu eleições indiretas para Presidência da República.\n\n() Ainda no governo de Castelo Branco foi decretado o Ato Institucional nº 4 (AI-4) que definiu as orientações que nortearam a elaboração da nova Constituição, outorgada em janeiro de 1967.\n\n() No governo Costa e Silva, políticos cassados pela ditadura, estudantes e trabalhadores de diversas categorias aliaram-se para formar a \"Frente Ampla\".\n\nAssinale a alternativa que apresenta a sequência CORRETA:",
      subItens: []
    },
    options: ["VFVFF.","VVFVV.","FFVFV.","FVVVF."],
    correctAnswer: 1,
    materia: "História – Ditadura Militar",
    conteudo: "História – História do Brasil Contemporâneo",
    imageNames: []
  }),

createQuestion({
    id: 20,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 20\nAnalise as afirmativas a seguir, que fazem um balanço do Brasil Contemporâneo, dialogam interdisciplinarmente com a Geografia e se referem aos principais desafios a serem superados pelo país no século XXI.\n\nI- Foi apresentado um pacote de ajustes fiscais pelo governo em 2015, buscando corrigir os excessos da política de gastos do período anterior; porém, apesar de apresentar diminuição significativa dos índices de pobreza, o país ainda mantém mais de 16 milhões de pessoas vivendo na pobreza extrema.\n\nII- A inserção do Brasil na economia globalizada trouxe custos sociais elevados, sendo um deles o desemprego que atingiu principalmente os centros industriais e setores com grande automação como o bancário. Nesse contexto, a estimativa em 2014 da População Economicamente Ativa (PEA) era de 24,2 milhões de pessoas, sendo que, dessas, aproximadamente 5% estavam desempregadas.\n\nIII- No quesito educação, o Programa \"Todos pela Educação\" do MEC estipulou 5 metas para 2022: 98% dos indivíduos em idade escolar deverão estar frequentando a escola; 100% deverão saber satisfatoriamente ler, escrever e calcular no ensino fundamental; 70% terão aprendido o conteúdo referente a seu ano; 95% dos jovens de 16 anos terão completado o ensino fundamental, e 90% dos de 19 anos, o ensino médio, sendo que o investimento público em educação básica será de 5% do PIB brasileiro.\n\nEstá(ão) CORRETA(S) a(s) afirmativa(s):",
      subItens: []
    },
    options: ["I apenas.","III apenas.","I e II apenas.","I, II e III."],
    correctAnswer: 3,
    materia: "História – Brasil Contemporâneo",
    conteudo: "Geografia – Geografia Econômica",
    imageNames: []
  }),

createQuestion({
    id: 21,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 21\nObserve a imagem a seguir:\n\nHere's the roomy G-E Refrigerator my family won't outgrow\nDisponível em:\nhttps://www.flickr.com/photos/25152449@$1_{06}_{}$/3007836828.\nAcesso: 11 dez. 2018.\n\nA imagem faz parte de um anúncio publicitário de refrigerador, publicado na revista feminina estadunidense McCall's no ano de 1952, que buscava evidenciar que, EXCETO:",
      subItens: []
    },
    options: ["A economia dos Estados Unidos sinalizava um pequeno crescimento durante a Segunda Guerra Mundial e, de forma não tão arrojada, continuaria a crescer nas décadas seguintes.","A fartura de alimentos, a geladeira cheia de produtos e a atmosfera de alegria denotavam a satisfação causada pela compra do refrigerador, padrão novo que podia ser desfrutado pelos trabalhadores daquele país à época.","Os bens, que antes eram restritos a uma pequena parcela da população estadunidense, passaram a ser acessíveis à maioria das pessoas daquele país.","A representação sugeria um ideal de felicidade baseado no consumismo e na posse de bens como prova de sucesso social e econômico, que foi disseminado na sociedade do Estados Unidos e de seus parceiros econômicos, incluindo o Brasil."],
    correctAnswer: 0,
    materia: "História – Pós-Segunda Guerra Mundial",
    conteudo: "História – Sociedade de Consumo",
    imageNames: []
  }),

createQuestion({
    id: 22,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 22\nDurante a Conferência de São Francisco, realizada em junho de 1945 nos Estados Unidos, representantes de 50 países redigiram a carta das Nações Unidas, que daria origem, no mesmo ano, à Organização das Nações Unidas (ONU).\n\nSobre a ONU e sua atuação, é INCORRETO afirmar que:",
      subItens: []
    },
    options: ["Os anos da Guerra Fria, sucessores aos da criação da ONU, por dividirem o mundo em dois polos de influência distintos, limitaram o papel da organização no campo de segurança internacional.","A ação da ONU, devido à influência da Guerra Fria, se restringiu a intervenções de paz apenas quando estas não colidiam com os interesses das grandes potências que a manipulavam, assistindo passivamente, devido aos vetos do Conselho de Segurança, a conflitos que resultaram em milhares de mortos.","A primeira e maior missão de observação da ONU, que pedia um cessar-fogo em 1948, ocorreu na região da Coreia do Sul e da Coreia do Norte.","O Conselho de Segurança da ONU é atualmente composto por cinco membros permanentes e dez membros eleitos de dois em dois anos. Os membros permanentes são: Estados Unidos, Rússia, Inglaterra, França e China."],
    correctAnswer: 2,
    materia: "História – Relações Internacionais",
    conteudo: "História – Pós-Segunda Guerra Mundial",
    imageNames: []
  }),

createQuestion({
    id: 23,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 23\nCom o fim do governo de Floriano Peixoto, em 1894, o Brasil passou a ser governado por uma Oligarquia. Durante a República Oligárquica, o Partido Republicano Mineiro (PRM) e o Partido Republicano Paulista (PRP) indicavam e sustentavam o presidente de acordo com seus interesses, afora algumas exceções.\n\nSobre essas exceções, relacione a COLUNA I que apresenta o nome do presidente com a descrição de sua atuação da COLUNA II.\n\nCOLUNA I\n\$1_{1}$- Afonso Pena (1906 – 1909).\n\$1_{2}$- Nilo Peçanha (1909-1910).\n\$1_{3}$- Venceslau Brás (1914 – 1918).\n\$1_{4}$- Artur Bernardes (1922 – 1926).\n\$1_{5}$- Washington Luís (1926 – 1930).\n\nCOLUNA II\n\nÚltimo presidente da oligarquia, fluminense, conhecido como \"paulista de Macaé\", seu lema foi “governar é abrir estradas\".\n\nEsse mineiro sucedeu Hermes da Fonseca, restabelecendo a política do \"café com leite\", seu governo foi marcado por uma grande guerra.\n\nO governo desse mineiro foi marcado pela oposição dos militares de baixa patente, o que o levou a decretar estado de sítio inúmeras vezes.\n\nPresidente mineiro que pôs em prática a política de valorização do café estabelecida no Convênio de Taubaté. Não concluiu seu governo devido à morte súbita.\n\nAfiliado ao Partido Republicano Fluminense, assumiu a presidência após disputa acirrada entre candidato gaúcho e baiano.\n\nAssinale a alternativa que apresenta a sequência CORRETA:",
      subItens: []
    },
    options: ["53412.","3 2 1 4 5.","4 15 2 3.","2 4 3 5 1."],
    correctAnswer: 0,
    materia: "História – República Oligárquica",
    conteudo: "História – Presidentes do Brasil",
    imageNames: []
  }),

createQuestion({
    id: 24,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 24\nLeia o fragmento a seguir:\n\"Então, quando o cabo do braçalote gemeu, encapelando-se no Cais da verga até os cotovelos do horizonte sem fim, sua memória também rangeu com os dentes da catraca, e pôde lembrar-se de quando estourara a notícia da prisão [...]: o outrora arguto Conde de Oeiras fora considerado traidor e condenado à morte. Subira ao trono [...] e com ela todo o desencanto com a monarquia.\"\nFonte: BARRETO, Antônio. A barca dos amantes. 4ª ed. Belo Horizonte[MG]: Lê, p. 27. 2018. [Fragmento: Adaptado]\n\nO fragmento de Antônio Barreto, na obra “A barca dos amantes”, em suas lacunas, retrata personagens históricos como, EXCETO:",
      subItens: []
    },
    options: ["Sebastião José de Carvalho e Melo, vulgo Marquês de Pombal, pertencente à pequena nobreza portuguesa, homem que mudou o destino de Portugal e que foi preso conforme disposto no fragmento.","Maria Francisca Isabel Josefa Antónia Gertrudes Rita Joana, vulgo D. Maria I a louca, primeira mulher a assumir o trono Português.","D. José I, rei de Portugal, que teve um reinado glorioso, graças aos esforços, à iniciativa enérgica e à notável persistência de Sebastião José de Carvalho na administração de que se encarregara.","Marquês de Pombal que muito conseguiu com sua administração enérgica, a despeito dos inimigos, da nobreza e da poderosa Companhia de Jesus que, por meio de muitas intrigas, tentavam acabar com seu prestígio junto ao rei."],
    correctAnswer: 2,
    materia: "História – Período Colonial",
    conteudo: "Literatura Brasileira – Obras Literárias",
    imageNames: []
  }),

createQuestion({
    id: 25,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "MATEMÁTICA\nQUESTÃO 25\nNo ano de 2018, foi realizada uma pesquisa, utilizando-se questionários sobre educação. Nessa pesquisa, João, Alfredo e Enéias tabularam as respostas dos questionários, respondidos pelos usuários de uma determinada universidade. Sabendo-se que João tabulou um quarto do total de questionários, Alfredo tabulou três quintos do que sobrou e Enéias tabulou os 1020 questionários restantes, a diferença entre os números de questionários tabulados por Enéias e João foi de:",
      subItens: []
    },
    options: ["170.","150.","120.","100."],
    correctAnswer: 0,
    materia: "Matemática – Frações",
    conteudo: "Matemática – Resolução de Problemas",
    imageNames: []
  }),

createQuestion({
    id: 26,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 26\nEm uma apresentação na escola, oito amigos, entre eles Carlos, Timóteo e Joana, formam uma fila.\n\nCalcule o número de diferentes formas que esta fila de amigos pode ser formada de modo que Carlos, Timóteo e Joana fiquem sempre juntos:",
      subItens: []
    },
    options: ["8!","5!.3!","6! .3!","8! .3!"],
    correctAnswer: 2,
    materia: "Matemática – Análise Combinatória",
    conteudo: "Matemática – Permutações",
    imageNames: []
  }),

createQuestion({
    id: 27,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 27\nEm um novo projeto de iniciação científica, o estudante de designer João está fabricando uma peça para exposição, na qual ele utiliza uma placa quadrada de um metal cuja densidade é de 9000Kg/m³, com 5000mm de lado. Sabe-se que essa chapa tem a espessura de 2,5cm e que inicialmente o profissional recorta um círculo de diâmetro máximo. Em seguida, ele recorta desse círculo um quadrado de dimensão máxima.\n\nA massa em toneladas desse quadrado recortado é de aproximadamente:",
      subItens: []
    },
    options: ["2,66ton.","2,71ton.","2,76ton.","2,81ton."],
    correctAnswer: 3,
    materia: "Matemática – Geometria Plana",
    conteudo: "Matemática – Conversão de Unidades",
    imageNames: []
  }),

createQuestion({
    id: 28,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 28\nO preço do litro de determinado produto de limpeza é igual a R$ 0,32. Se um reservatório industrial tem a forma de um paralelepipedo retângulo reto, medindo internamente 1,2dam × 125cm × 0,08hm, então o preço que se pagará para encher esse reservatório com o referido produto de limpeza será igual a:",
      subItens: []
    },
    options: ["R$32.450,00.","R$35.400,00.","R$38.400,00.","R$40.450,00."],
    correctAnswer: 2,
    materia: "Matemática – Geometria Espacial",
    conteudo: "Matemática – Volume",
    imageNames: []
  }),

createQuestion({
    id: 29,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 29\nJoaquim, um jovem empreendedor, estuda duas possibilidades para investir R$10.000,00. A primeira opção é aplicar durante meio ano a uma taxa de juros simples de 0,5% a.m. e a segunda, aplicar o mesmo montante a uma taxa de juros compostos.\n\nAssinale a alternativa que apresenta a taxa de juros compostos ao mês para que, com a mesma duração e com o mesmo montante inicial, Joaquim obtenha o mesmo rendimento da primeira possibilidade:\n\n(Dados:\$1_{1}$,18 = 102797 * 10−5;1,03 = 1004939 * 10-6 ;)",
      subItens: []
    },
    options: ["2,797% a.m.","1,555% a.m.","0,352% a.m.","0,4939% a.m."],
    correctAnswer: 3,
    materia: "Matemática – Matemática Financeira",
    conteudo: "Matemática – Juros Simples e Compostos",
    imageNames: []
  }),

createQuestion({
    id: 30,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 30\nDurante o período de final de ano, determinado lojista decidiu aumentar o preço original de um produto em 12,5% e, no início de janeiro, decidiu liquidar e dar um desconto de 12,5% sobre o preço reajustado. Então, relativamente ao preço original, o preço final do produto sofreu uma redução aproximada de:",
      subItens: []
    },
    options: ["0%.","1%.","1,5%.","2%."],
    correctAnswer: 2,
    materia: "Matemática – Porcentagem",
    conteudo: "Matemática – Aumentos e Descontos",
    imageNames: []
  }),

createQuestion({
    id: 31,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUÍMICA\nQUESTÃO 31\nObserve o aparelho esquematizado a seguir:\n\nGerador\nLâmpada\n\nAssinale a alternativa que indica CORRETAMENTE a substância que, quando adicionada em quantidade adequada à água, poderia provocar um brilho muito intenso na lâmpada.",
      subItens: []
    },
    options: ["AgCl.","$1_{4}$","$1_{3}$","FeS."],
    correctAnswer: 2,
    materia: "Química – Eletroquímica",
    conteudo: "Química – Eletrólitos",
    imageNames: []
  }),

createQuestion({
    id: 32,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 32\nVinagre é uma mistura homogênea cujo principal constituinte é o ácido acético. Um estudante de química analisou uma amostra de uma garrafa de 500mL de vinagre de maçã, em que, no rótulo, há a informação de que o teor do ácido acético presente na solução é de 4,2% m/v.\n\nConsiderando que o ácido acético é o único composto de caráter ácido do vinagre, analise as proposições sugeridas pelo estudante após as análises.\n\nI- A molaridade do ácido acético na amostra analisada é 0,7mol/L.\nII- Ao titular 20mL desse vinagre com hidróxido de sódio 1mol/L, foram gastos 50mL da base.\nIII- Uma cozinheira que utiliza 6,3g de vinagre por dia, durante 30 dias, irá utilizar 9 garrafas.\n\nEstá(ão) CORRETA(S) a(s) afirmativa(s):",
      subItens: []
    },
    options: ["I apenas.","I e II apenas.","II e III apenas.","I e III apenas."],
    correctAnswer: 3,
    materia: "Química – Soluções",
    conteudo: "Química – Estequiometria",
    imageNames: []
  }),

createQuestion({
    id: 33,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 33\nÓleos essenciais são compostos aromáticos voláteis extraídos de plantas que são utilizados na aromaterapia devido às suas propriedades analgésicas, relaxantes e estimulantes.\n\nAs fórmulas estruturais de alguns componentes dos óleos essenciais são apresentadas a seguir:\n\nlinalol\neugenol\ncitronelal\nanetol\n\nConsiderando as fórmulas apresentadas, assinale a alternativa CORRETA:",
      subItens: []
    },
    options: ["A reação de oxidação da hidroxila do linalol com $1_{4}$ em meio ácido produzirá cetona.","Um teste apropriado para diferenciar o citronelal e o anetol é o reagente de Tollens.","Um teste apropriado para diferenciar o eugenol e o anetol é o teste da solução de bromo ($1_{2}_{}$/$1_{4}$).","A reação de oxidação da hidroxila do eugenol com $1_{4}$ em meio ácido produzirá cetona."],
    correctAnswer: 1,
    materia: "Química – Química Orgânica",
    conteudo: "Química – Funções Orgânicas",
    imageNames: []
  }),

createQuestion({
    id: 34,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 34\nUma fábrica de sucos realizou analises físico-químicas em um laboratório de controle de qualidade do suco de limão com manjericão e do suco de tomate e obteve os seguintes resultados:\n\n• Suco de limão com manjericão: pH = 2,3.\n• Suco de tomate: pH = 4,3.\n\nDados: log 5= 0,7\n\nCom base nos resultados, é CORRETO afirmar que:",
      subItens: []
    },
    options: ["O suco de limão com manjericão é 2 vezes mais ácido que o suco de tomate.","A concentração de OH - nos dois sucos é igual a zero.","No suco de tomate a [H+]/[ OH-] = 1.","A concentração de H⁺ no suco de limão com manjericão é igual a 5 X 10-3 mol/L."],
    correctAnswer: 3,
    materia: "Química – Físico-Química",
    conteudo: "Química – pH e pOH",
    imageNames: []
  }),

createQuestion({
    id: 35,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 35\nNicotina, um dos principais constituintes do cigarro, é um alcaloide, encontrado nas folhas do tabaco (Nicotiana tabacum), planta originária das Américas, sendo a molécula responsável pela dependência.\n\nSua composição porcentual, em massa, é 74,1% de carbono, 8,6% de hidrogênio e 17,3% de nitrogênio.\n\nAssinale a alternativa que indica CORRETAMENTE a fórmula mínima da nicotina.",
      subItens: []
    },
    options: ["C5H7N","C3H3N","C10H14N2","C6H8N"],
    correctAnswer: 0,
    materia: "Química – Estequiometria",
    conteudo: "Química – Fórmulas Químicas",
    imageNames: []
  }),

createQuestion({
    id: 36,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 36\nMedicamento é um produto farmacêutico, tecnicamente obtido ou elaborado, com finalidade profilática, curativa, paliativa ou para fins de diagnóstico. A química orgânica é fundamental para o desenvolvimento de novos fármacos e o crescimento da indústria farmacêutica.\n\nDois dos princípios ativos de medicamentos mais utilizados pelos brasileiros são ilustrados a seguir:\n\nSildenafil\n\nFluoxetina\n\nEm relação aos compostos apresentados, assinale a alternativa CORRETA:",
      subItens: []
    },
    options: ["A fluoxetina não possui atividade óptica.","As funções orgânicas presentes no sildenafil são amina, amida, éter e tiol.","A fórmula molecular da fluoxetina é C17H18F3NO.","A fórmula molecular do sildenafil é C22H28N6O4S."],
    correctAnswer: 2,
    materia: "Química – Química Orgânica",
    conteudo: "Química – Funções Orgânicas",
    imageNames: []
  }),

createQuestion({
    id: 37,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "BIOLOGIA\nQUESTÃO 37\nNa espécie humana existem algumas características que obedecem à primeira lei de Mendel. Dentre essas características, é possível citar: a polidactilia e as sardas no rosto.\n\nClarício, que apresenta polidactilia e sardas no rosto, casa-se com Josefina, cujo fenótipo é igual ao do seu marido. Depois de uma análise genética de seus pais, foi determinado que o casal em questão é diíbrido para os dois fenótipos citados.\n\nConsiderando essas informações, determine a probabilidade de o casal ter uma filha com os mesmos fenótipos citados:",
      subItens: []
    },
    options: ["1/32.","9/32.","9/16.","1/16."],
    correctAnswer: 1,
    materia: "Biologia – Genética",
    conteudo: "Biologia – Leis de Mendel",
    imageNames: []
  }),

createQuestion({
    id: 38,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 38\nO sistema urinário humano ou aparelho urinário é encarregado pela preservação da homeostasia corpórea. Esse controle se dá através da identificação da condição do organismo: se, por exemplo, há uma queda ou um aumento da pressão sanguínea, o Sistema Nervoso Central (SNC) ativa mecanismos renais que trabalharão com a finalidade de normalizá-la. Os órgãos que compõem o sistema urinário são os rins e as vias urinárias. As vias urinárias compreendem a uretra, a bexiga e o ureter.\n\nSobre o sistema urinário humano, é INCORRETO afirmar que:",
      subItens: []
    },
    options: ["Os ureteres são dois tubos de aproximadamente 20cm de comprimento cada, que conduzem a urina dos rins para a bexiga.","A uretra é o canal que conduz a urina do interior da bexiga para fora do corpo. Ela termina no pênis ou na vulva.","A bexiga urinária recebe e armazena temporariamente a urina com uma quantidade que pode variar de 400ml até 500ml.","Os rins funcionam como um filtro que retém as impurezas do sangue venoso e o deixa em condições de circular pelo organismo."],
    correctAnswer: 3,
    materia: "Biologia – Anatomia Humana",
    conteudo: "Biologia – Sistema Urinário",
    imageNames: []
  }),

createQuestion({
    id: 39,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 39\nAs populações possuem diversas características próprias, mensuráveis.\n\nCada membro de uma população pode nascer, crescer e morrer, mas somente uma população como um todo possui taxas de natalidade e de crescimento específicas, além de possuir um padrão de dispersão no tempo e no espaço.\n\nO tamanho de uma população pode ser avaliado pela sua densidade.\n\nA densidade populacional pode sofrer alterações. Mantendo-se fixa a área de distribuição, a população pode aumentar devido a nascimentos e imigrações. A diminuição da densidade pode ocorrer como consequência de mortes ou de emigrações.\n\nDisponível em:\nhttps://www.sobiologia.com.br/conteudos/bio_ecologia/$1_{16}$.php Acesso: 11 de dez. 2018.\n\nO gráfico a seguir representa a curva de crescimento de uma população a partir de um pequeno número de indivíduos:\n\nNº de indivíduos\nI\nIV\nII\nIII\nTempo\nFONTE: AMABIS, J. M.; MARTHO, G. R. Biologia em contexto.\nVolume único.1ª. ed. São Paulo: Moderna. 2013, p. 77 (Adaptado).\n\nSobre o gráfico, é INCORRETO afirmar que:",
      subItens: []
    },
    options: ["A curva I representa o crescimento intrínseco que é a capacidade teórica de crescimento de uma população biológica.","A linha II representa o tamanho populacional máximo suportado pelo ambiente ou carga biótica máxima do meio.","A curva III representa o crescimento populacional que é o resultado da interação entre a taxa de crescimento intrínseco e a resistência do meio.","A área IV representa a resistência ambiental, trata-se meramente dos fatores abióticos que limitam o crescimento populacional."],
    correctAnswer: 3,
    materia: "Biologia – Ecologia",
    conteudo: "Biologia – Dinâmica de Populações",
    imageNames: []
  }),

createQuestion({
    id: 40,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 40\nHá mais de 100 anos, em 1904, aconteceu a primeira campanha de vacinação em massa feita no Brasil. Idealizada por Oswaldo Cruz, o fundador da saúde pública no país, a campanha tinha o objetivo de controlar a varíola, que então dizimava boa parte da população do Rio de Janeiro. Porém, as ações de imunização eram caracterizadas pela descontinuidade e pela baixa área de cobertura.\n\nAtualmente, são oferecidas à população brasileira 19 vacinas recomendadas pela Organização Mundial da Saúde (OMS). São mais de 300 milhões de doses, por ano, todas adquiridas pelo Ministério da Saúde e distribuídas aos estados e municípios visando à imunização de crianças, adolescentes, adultos e idosos.\nDisponível em: http://www.blog.saude.gov.br/index.php/promocao-da-saude/ Acesso: 11 dez. 2018 (Fragmento: Adaptado).\n\nOs cientistas estão em constante pesquisa para desenvolverem vacinas para a imunização da população humana contra diversos agentes causadores que podem ser vírus, bactérias, protozoários, fungos e outros.\n\nRelacione os agentes causadores enumerados na COLUNA I com as doenças apresentados na COLUNA II:\n\nCOLUNA I\$1_{1}$- Protozoários.\$1_{2}$- Fungos.\$1_{3}$- Bactérias.\$1_{4}$- Vírus.\n\nCOLUNA II\n() Tracoma.\n() Hidrófobia.\n() Hanseníase.\n() Histoplasmose.\n() Toxoplasmose.\n() Caxumba.\n\nAssinale a alternativa que apresenta a sequência CORRETA:",
      subItens: []
    },
    options: ["434213.","344124.","3 4 3 2 1 4.","4 4 3 2 1 2."],
    correctAnswer: 2,
    materia: "Biologia – Imunologia",
    conteudo: "Biologia – Doenças Infecciosas",
    imageNames: []
  }),

createQuestion({
    id: 41,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 41\nOs vegetais multicelulares são formados por um grupamento de células diferenciadas que exercem uma mesma função. Esses conjuntos celulares são designados como tecidos e a divisão da Biologia que os estuda é denominada de Histologia.\n\nNesses tecidos, existem células que possuem celulose em sua parede celular, vacúolos e cloroplastos em seu interior.\n\nAssinale a alternativa que apresenta o tecido vegetal formado por células mortas, alongadas e de parede celular lignificada.",
      subItens: []
    },
    options: ["Xilema.","Floema.","Colênquima.","Meristema."],
    correctAnswer: 0,
    materia: "Biologia – Botânica",
    conteudo: "Biologia – Tecidos Vegetais",
    imageNames: []
  }),

createQuestion({
    id: 42,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 42\nO Reino Animal, Animalia ou Metazoa é constituído por espécimes heterótrofos que necessitam ingerir ou absorver moléculas orgânicas pré-formadas de outros seres vivos para aquisição de energia e síntese das moléculas de que precisam. Os seres que pertencem ao Reino Animal são eucariontes e pluricelulares. Eles possuem capacidade de locomoção e de reprodução sexuada. Os animais vertebrados (que possuem vértebras) e os animais invertebrados (que não possuem vértebras) são classificados em diversos filos.\n\nSobre os animais invertebrados e os vertebrados, analise as afirmativas a seguir:\n\nI- Nos artrópodes e na maioria dos moluscos, o sistema circulatório é aberto (lacunar), ou seja, o líquido bombeado pelo coração periodicamente abandona os vasos e cai em lacunas corporais.\n\nII- Nos artrópodes e na maioria dos insetos, a respiração é traqueal; nos aracnídeos, além da traqueal, também é observada a filotraqueal; e os crustáceos em geral respiram por brânquias.\n\nIII- Nos anelídeos e nos vertebrados, o sistema circulatório é fechado, o sangue circula por uma grande rede de vasos pelos quais ocorrem as trocas de substâncias entre o sangue e os tecidos.\n\nIV- Nos vertebrados, o sistema respiratório pode ser pulmonar ou branquial, ou seja, os processos de trocas gasosas ocorrem nos pulmões ou nas brânquias.\n\nEstão CORRETAS as afirmativas:",
      subItens: []
    },
    options: ["I, II, III e IV.","I e II apenas.","I, II e III apenas.","I, II e IV apenas."],
    correctAnswer: 2,
    materia: "Biologia – Zoologia",
    conteudo: "Biologia – Fisiologia Animal",
    imageNames: []
  }),

createQuestion({
    id: 43,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "GEOGRAFIA\nQUESTÃO 43\nLeia o fragmento a seguir:\n\"Consiste na continuidade da área continental emersa e pode atingir uma profundidade de cerca de 200m. Caracteriza-se por ser uma planície submersa que margeia todos os continentes, em uma extensão que varia de 70 a 1000km\".\nFonte: MARTINS, DADA et. al. Geografia no cotidiano: ensino médio. Curitiba. Base Editorial. 2016. p. 118.\n\nO fragmento se refere à forma de relevo submarino, cujo nome é:",
      subItens: []
    },
    options: ["Planície Abissal.","Fossa Oceânica.","Talude Continental.","Plataforma Continental."],
    correctAnswer: 3,
    materia: "Geografia – Geomorfologia",
    conteudo: "Geografia – Relevo Submarino",
    imageNames: []
  }),

createQuestion({
    id: 44,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 44\nSabemos que ao longo de bilhões de anos, a Terra passou por diferentes transformações que vão desde o resfriamento e solidificação das camadas até os resultados das transformações antrópicas.\n\nNesse contexto, assinale V para as afirmativas verdadeiras e F para as falsas.\n\n() A Era Pré-Cambriana caracterizou-se pela inexistência da vida no planeta e pela constituição das primeiras rochas magmáticas.\n\n() A Era Paleozoica caracterizou-se pela formação das grandes cadeias de montanha, tais como os Andes e os Alpes.\n\n() A Era Mesozoica foi marcada pela fragmentação do continente Gondwana, que resultou na formação dos continentes africano e sul-americano e do oceano Atlântico.\n\n() A Era Cenozoica foi marcada pelo grande soterramento de florestas em diversas partes do globo, que resultou na formação da jazidas de carvão mineral.\n\nAssinale a alternativa que apresenta a sequência CORRETA:",
      subItens: []
    },
    options: ["FVFV.","VFVF.","VVFV.","VFFF."],
    correctAnswer: 1,
    materia: "Geografia – Geologia",
    conteudo: "Geografia – Eras Geológicas",
    imageNames: []
  }),

createQuestion({
    id: 45,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 45\nSão acontecimentos do período entre guerras e oriundos da Segunda Guerra Mundial, EXCETO:\n\nA) No período entre guerras ocorreu, na Itália, a ascensão do fascismo que foi marcado por um nacionalismo exacerbado, uma política de controle sindical e uma perseguição à imprensa.\n\nB) No decorrer da Segunda Guerra Mundial, o Japão, que tinha a intenção de ampliar seu poder na Ásia, formou um bloco com França e Inglaterra, denominado bloco dos Aliados.\n\nC) Durante a década de 1920, a Alemanha teve uma crise econômica de grandes proporções, que resultou na diminuição da arrecadação do Estado e no aumento do desemprego.\n\nD) Os alemães invadiram o território francês em 1940, forçando os franceses a assinarem um armistício que foi extremamente desastroso para a França.",
      subItens: []
    },
    options: ["No período entre guerras ocorreu, na Itália, a ascensão do fascismo que foi marcado por um nacionalismo exacerbado, uma política de controle sindical e uma perseguição à imprensa.","No decorrer da Segunda Guerra Mundial, o Japão, que tinha a intenção de ampliar seu poder na Ásia, formou um bloco com França e Inglaterra, denominado bloco dos Aliados.","Durante a década de 1920, a Alemanha teve uma crise econômica de grandes proporções, que resultou na diminuição da arrecadação do Estado e no aumento do desemprego.","Os alemães invadiram o território francês em 1940, forçando os franceses a assinarem um armistício que foi extremamente desastroso para a França."],
    correctAnswer: 1,
    materia: "História – Período Entre Guerras",
    conteudo: "História – Segunda Guerra Mundial",
    imageNames: []
  }),

createQuestion({
    id: 46,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 46\nLeia o fragmento a seguir:\n\"[Essa] é uma fonte de energia limpa, simples de ser obtida e que pode solucionar também parte do problema da quantidade de lixo que é descartado. Trata-se de uma mistura gasosa de metano e dióxido de carbono a partir da decomposição de restos orgânicos. Uma das formas de acelerar esse processo biológico é por meio de uso de biodigestores”.\nFonte: BALDRAIS, André. Ser protagonista – geografia. São Paulo. Edições SM. 2016. p. 66.\n\nO trecho se refere a um tipo de energia alternativa denominada:",
      subItens: []
    },
    options: ["Biogás.","Eólica.","Solar.","Nuclear."],
    correctAnswer: 0,
    materia: "Geografia – Fontes de Energia",
    conteudo: "Geografia – Meio Ambiente",
    imageNames: []
  }),

createQuestion({
    id: 47,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 47\nLeia o fragmento a seguir:\n\"[Esse grupo é formado por] países emergentes de grande expressão populacional e territorial, que, por apresentarem grande crescimento econômico e recursos produtivos, passaram a participar com maior intensidade da dinâmica global [especialmente, na primeira década do século XXI]. (...). Esse grupo já demonstrou que tem força econômica capaz de impor seus interesses no cenário global [pois] dispõem de consideráveis recursos econômicos, humanos e naturais e têm obtido maior protagonismo político diante de organismos internacionais, como o FMI e o Banco Mundial. [Visando ampliar a cooperação entre seus membros foi criado um banco de reservas emergenciais para socorro econômico entre os participantes, se necessário] \".\nFonte: BALDRAIS, André. Ser protagonista – geografia. São Paulo. Edições SM. 2016. p. 115. Adaptado.\n\nO trecho descreve o grupo, cuja sigla é:",
      subItens: []
    },
    options: ["SADC.","ASEAN.","BRICS.","APEC."],
    correctAnswer: 2,
    materia: "Geografia – Geopolítica",
    conteudo: "Geografia – Blocos Econômicos",
    imageNames: []
  }),

createQuestion({
    id: 48,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTÃO 48\nLeia o fragmento a seguir:\n“A revolta já tinha mais de quatro meses de vida e as vantagens do governo eram problemáticas. No Sul, a insurreição chegava às portas de São Paulo, e só a Lapa resistia tenazmente, uma das poucas páginas dignas e limpas de todo aquele enxurro de paixões. A pequena cidade tinha dentro de suas trincheiras o Coronel Gomes Carneiro, uma energia, uma vontade, verdadeiramente isso, porque era sereno, confiante e soube tornar verdade a gasta frase grandiloquente: resistir até a morte.\nA ilha do Governador tinha sido ocupada e Magé tomado; os revoltosos, porém, tinham a vasta baía e a barra apertada, por onde saíam e entravam, sem temer o estorvo das fortalezas.\nAs violências, os crimes que tinham assinalado esses dois marcos de atividade guerreira do governo, chegavam ao ouvido de Quaresma e ele sofria.\nDa ilha do Governador fez-se uma verdadeira mudança de móveis, roupas e outros haveres. O que não podia ser transplantado era destruído pelo fogo e pelo machado.\nA ocupação deixou lá a mais execranda memória e até hoje os seus habitantes ainda se recordam dolorosamente de um capitão, patriótico ou da guarda nacional, Ortiz, pela sua ferocidade e insofrido gosto pelo saque e outras vexações”.\nFonte: BARRETO, Lima. Triste fim de Policarpo Quaresma. Rio de Janeiro. Bestbolso. 2013. p. 211.\n\nO trecho expressa elementos que fazem parte das diferentes formas de produção e organização do espaço brasileiro ao longo de sua construção histórica e humana. A partir de uma perspectiva da geografia humana, é CORRETO afirmar que o trecho ressalta a Revolta da",
      subItens: []
    },
    options: ["Chibata.","Armada.","Cabanagem.","Vacina."],
    correctAnswer: 1,
    materia: "História – Primeira República",
    conteudo: "História – Revolta da Armada",
    imageNames: []
  }),

createQuestion({
    id: 49,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "LÍNGUA ESTRANGEIRA – INGLÊS\nRead the text to answer questions 49 to 53.\nFire Devastates Brazil's Oldest Science Museum\nThe overnight inferno likely claimed fossils, cultural artifacts, and more irreplaceable collections amassed over 200 years.\nBy Michael Greshko\nPUBLISHED September 6, 2018\nMajor pieces of Brazil's scientific and cultural heritage went up in smoke on September 2, as a devastating fire ripped through much of Rio de Janeiro's Museu Nacional, or National Museum. Founded in 1818, the museum is Brazil's oldest scientific institution and one of the largest and most renowned museums in Latin America, amassing a collection of some 20 million scientifically and culturally invaluable artifacts.\nThe Museu Nacional's holdings include Luzia, an 11,500-year-old skull considered one of South America's oldest human fossils, as well as the bones of uniquely Brazilian creatures such as the long-necked dinosaur Maxakalisaurus. Because of the auction tastes of Brazil's 19th-century emperors, the Museu Nacional also ended up with Latin America's oldest collection of Egyptian mummies and artifacts.\nEven the building holds historical importance: It housed the exiled Portuguese royal family from 1808 to 1821, after they fled to Rio de Janeiro in 1807 to escape Napoleon. The complex also served as the palace for Brazil's post-independence emperors until 1889, before the museum collections were transferred there in 1902.\nIn an September 5 email, Museu Nacional curator Débora Pires wrote that the entomology and arachnology collections were completely destroyed, as was most of the mollusk collection. However, technicians had braved the fire to save 80 percent of the mollusk holotypes—the specimens that formally serve as the global references for a given species. The museum's vertebrate specimens, herbarium, and library were housed separately and survived the fire.\n(...)\nAn Irreplaceable Loss\nIt's not yet clear how the fire started, but it did begin after the museum was closed to the public, and no injuries have yet been reported. Firefighters worked through the night to douse the burnt-out shell of the main building, but it seems the blaze has already seared a gaping hole in many scientists' careers.\n\"The importance of the collections that were lost couldn't be overstated,\" says Luiz Rocha, a Brazilian ichthyologist now at the California Academy of Sciences who has visited the Museu Nacional several times to study its collections. “They were unique as it gets: Many of them were irreplaceable, there's no way to put a monetary value on it.\"\n\"In terms of [my] life-long research agenda, I'm pretty much lost,\" says Marcus Guidoti, a Brazilian entomologist finishing up his Ph.D. in a program co-run by Brazil's Federal University of Rio Grande do Sul. Guidoti studies lace bugs, an insect family with more than 2,000 species worldwide. The Museu Nacional held one of the world's largest lace bug collections, but the fire likely destroyed it and the rest of the museum's five million arthropod specimens. “Those type specimens can't be replaced, and they are crucial to understand the species,\" he says by text message. “If I was willing to keep working on this family in this region of the globe, this was definitely a big hit.\"\nPaleontologist Dimila Mothé, a postdoctoral researcher at the Federal University of the State of Rio de Janeiro, adds that the blows to science extend beyond the collections themselves. \"It's not only the cultural history, the natural history, but all the theses and research developed there,\" she says. \"Most of the laboratories there were lost, too, and the research of several professors. I'm not sure you can say the impact of what was lost.\"\nBrazil's indigenous knowledge also has suffered. The Museu Nacional housed world-renowned collections of indigenous objects, as well as many audio recordings of indigenous languages from all over Brazil. Some of these recordings, now lost, were of languages that are no longer spoken.\n\"I have no words to say how horrible this is,\" says Brazilian anthropologist Mariana Françozo, an expert on South American indigenous objects at Leiden University. \"The indigenous collections are a tremendous loss we can no longer study them, we can no longer understand what our ancestors did. It's heartbreaking.\"\nOn Monday, The Brazilian publication $1_{1}_{}$ Rio reported that ashes of burned documents—some still flecked in notes or illustrations—have rained down from the sky more than a mile away from the Museu Nacional, thrown aloft by the inferno.\n(...)\nEditor's Note: This story was updated on September 6, 2018, with new details about which artifacts survived the fire.\nTaken from: https://www.nationalgeographic.com/science/2018/09/news-museu-nacional-fire-rio-de-janeiro-natural-history/. Access: 11 dez. 2018.\n\nIn the excerpt “Founded in 1818, the museum is Brazil's oldest scientific institution and one of the largest and most renowned museums in Latin America\" we have 3 (three) occurrences of:",
      subItens: []
    },
    options: ["The superlative of inferiority of adjectives.","The superlative of superiority of adjectives.","The comparative of inferiority of adjectives.","The comparative of superiority of adjectives."],
    correctAnswer: 1,
    materia: "Inglês – Gramática",
    conteudo: "Inglês – Superlativo",
    imageNames: []
  }),

createQuestion({
    id: 50,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTION 50\nIn the excerpt \"Even the building holds historical importance: It housed the exiled Portuguese royal family from 1808 to 1821, after they fled to Rio de Janeiro in 1807 to escape Napoleon\", the word IT refers to:",
      subItens: []
    },
    options: ["The building.","Importance.","Royal family.","Rio de Janeiro."],
    correctAnswer: 0,
    materia: "Inglês – Leitura e Interpretação",
    conteudo: "Inglês – Pronomes",
    imageNames: []
  }),

createQuestion({
    id: 51,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTION 51\nAccording to the text about the Brazilian National Museum, the fire\nA) burned vertebrate specimens, herbarium and library.\nB) destroyed entomology and arachnology collections.\nC) provoked the exile of the Portuguese royal family.\nD) reached renowned museums in Latin America.",
      subItens: []
    },
    options: ["burned vertebrate specimens, herbarium and library.","destroyed entomology and arachnology collections.","provoked the exile of the Portuguese royal family.","reached renowned museums in Latin America."],
    correctAnswer: 1,
    materia: "Inglês – Compreensão Textual",
    conteudo: "Inglês – Vocabulário",
    imageNames: []
  }),

createQuestion({
    id: 52,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTION 52\nThe fire caused damages and losses in different areas of the National Museum, EXCEPT in:\nA) Paleontology.\nB) Entomology.\nC) Herbology.\nD) Anthropology.",
      subItens: []
    },
    options: ["Paleontology.","Entomology.","Herbology.","Anthropology."],
    correctAnswer: 2,
    materia: "Inglês – Compreensão Textual",
    conteudo: "Inglês – Vocabulário",
    imageNames: []
  }),

createQuestion({
    id: 53,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTION 53\nThe correct reported speech form for the sentence \"Most of the laboratories there were lost, too, and the research of several professors. I'm not sure you can say the impact of what was lost”, said by Dimila Mothé, is:\nA) Dimila Mothé said that most of the laboratories there were lost, too, and the research of several professors and she is not sure one can say the impact of what was lost.\nB) Dimila Mothé said that most of the laboratories there had been lost, too, and the research of several professors and she was not sure one could say the impact of what had been lost.\nC) Dimila Mothé said that most of the laboratories there were lost, too, and the research of several professors and she was not sure one could say the impact of what was lost.\nD) Dimila Mothé said that most of the laboratories there are lost, too, and the research of several professors and she was not sure one can say the impact of what was lost.",
      subItens: []
    },
    options: ["Dimila Mothé said that most of the laboratories there were lost, too, and the research of several professors and she is not sure one can say the impact of what was lost.","Dimila Mothé said that most of the laboratories there had been lost, too, and the research of several professors and she was not sure one could say the impact of what had been lost.","Dimila Mothé said that most of the laboratories there were lost, too, and the research of several professors and she was not sure one could say the impact of what was lost.","Dimila Mothé said that most of the laboratories there are lost, too, and the research of several professors and she was not sure one can say the impact of what was lost."],
    correctAnswer: 1,
    materia: "Inglês – Gramática",
    conteudo: "Inglês – Discurso Indireto",
    imageNames: []
  }),

createQuestion({
    id: 54,
    university: "VESTIBULAR",
    year: 2019,
    text: {
      principal: "QUESTION 54\nLima Barreto's work (Sad End of Policarpo Quaresma) describes a man who acted in some areas along his life. Check the alternative that shows the area that it was NOT active.\nA) Agriculture.\nB) Music.\nC) Medicine.\nD) Politics.",
      subItens: []
    },
    options: ["Agriculture.","Music.","Medicine.","Politics."],
    correctAnswer: 2,
    materia: "Inglês – Vocabulário",
    conteudo: "Literatura Brasileira – Obras Literárias",
    imageNames: []
  }),